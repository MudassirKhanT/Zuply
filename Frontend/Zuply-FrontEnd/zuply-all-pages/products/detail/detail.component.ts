...
import { Product, Review, ReviewsResponse } from '../../../core/models';
...
export class DetailComponent implements OnInit {
  ...
  isCategory(...names: string[]): boolean {
    const cat = (this.product?.categoryName || this.product?.category || '').toLowerCase();
  ...
  loadReviews(productId: number): void {
    ...
    this.reviewService.getReviews(productId).subscribe({
      next: res => {
        if (res.success) this.reviews = res.data.reviews || [];
