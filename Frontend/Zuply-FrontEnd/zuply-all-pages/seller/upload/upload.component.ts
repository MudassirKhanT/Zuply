...
export class UploadComponent implements OnInit {
  ...
  private validateManual(): boolean {
    ...
    if (!this.form.deliveryMethod) { this.errorMsg = 'Please select a delivery method.'; return false; }
    if (!this.form.returnPolicy)   { this.errorMsg = 'Please select a return policy.';   return false; }
    return true;
  }
    ...
    this.listingService.generateListing(imageId).subscribe({
      next: async res => {
        if (res.success) {
          ...
          // Save stock immediately after generation
          try {
            await this.listingService.editListing(res.data.productId, {
              stock: this.form.stock ?? 0
            }).toPromise();
          } catch { /* non-critical — seller can set in preview */ }
      ...
      error: err => {
        ...
        if (err.status === 429 || serverMsg === 'AI_QUOTA_EXCEEDED'
            || serverMsg.toLowerCase().includes('quota')
            || serverMsg.toLowerCase().includes('exhausted')) {
          this.errorMsg = '⚠️ AI assistant has reached its daily limit. Please try again later, or switch to Manual mode to list your product.';
        } else {
          this.errorMsg = 'AI generation failed. Please try again or switch to Manual mode.';
        }
      }
    });
  }
