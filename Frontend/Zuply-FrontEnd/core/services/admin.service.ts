...
export class AdminService {
  ...
  approveProduct(id: number, source: string = 'LISTING'): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(
      `${this.API}/admin/products/${id}/approve?source=${source}`, {});
  }

  rejectProduct(id: number, source: string = 'LISTING'): Observable<ApiResponse<any>> {
    return this.http.patch<ApiResponse<any>>(
      `${this.API}/admin/products/${id}/reject?source=${source}`, {});
  }

  getOrders(): Observable<ApiResponse<any[]>> {
    return this.http.get<ApiResponse<any[]>>(`${this.API}/admin/orders`);
  }
