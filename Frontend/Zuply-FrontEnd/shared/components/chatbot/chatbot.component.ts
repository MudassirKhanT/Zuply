  ...
  send(): void {
    ...
    this.chatbot.send(text, [...this.history]).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        ...
        let msg = "⚠️ Couldn't reach the server. Please check your connection and try again.";
        if (err.status === 0) {
          msg = "⚠️ Cannot connect to server. Please try again in a moment.";
        } else if (err.status === 429) {
          msg = "⚠️ The AI assistant has reached its daily limit. Please try again tomorrow or later today.";
        } else if (err.status === 500 || err.status === 502) {
          const serverMsg: string = err.error?.message || '';
          if (serverMsg === 'AI_QUOTA_EXCEEDED' || serverMsg.includes('429') || serverMsg.toLowerCase().includes('quota')) {
            msg = "⚠️ The AI assistant has reached its daily limit. Please try again tomorrow.";
          } else {
            msg = "⚠️ The AI service encountered an error. Please try again in a moment.";
          }
