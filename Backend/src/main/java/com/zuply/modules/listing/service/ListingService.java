...
public class ListingService {
    ...
    public PublishResponse publishListing(Long productId, Long sellerId) {
        ...
        // Validate the listing has a price and stock before publishing
        ...
        if (product.getStock() == null || product.getStock() < 0) {
            throw new IllegalArgumentException(
                    "Cannot publish listing without stock quantity. Please set stock before publishing.");
        }
        ...
        if (!alreadyMirrored) {
            ...
            mainProduct.setStock(product.getStock());
            mainProduct.setImageUrl(product.getImageUrl());
            mainProduct.setExtraImages(product.getExtraImages());
            mainProduct.setDeliveryMethod(product.getDeliveryMethod());
            mainProduct.setReturnPolicy(product.getReturnPolicy());
