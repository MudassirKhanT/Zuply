...
public class PaymentController {
    ...
    public ResponseEntity<ApiResponse<PaymentOrderResponse>> createOrder(
        ...
        try {
            ...
            // Razorpay surfaces auth failures as "Authentication failed" or "api key provided is invalid"
            boolean isAuthError = msg.contains("Authentication failed")
                    || msg.contains("api key provided is invalid")
                    || msg.contains("BAD_REQUEST_ERROR");
            String userMsg = isAuthError
                    ? "Payment gateway is not configured correctly. Please use Cash on Delivery."
