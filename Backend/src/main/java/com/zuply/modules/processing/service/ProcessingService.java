...
public class ProcessingService {
    ...
    public void processImage(Long imageId) {
        ...
        try {
            if (image.getOriginalUrl() == null || image.getOriginalUrl().isBlank()) {
                throw new IOException("Image has no original URL stored");
            }

            boolean isRemote = image.getOriginalUrl().startsWith("http");

            if (isRemote) {
                // Production (Supabase): skip heavy in-memory processing to avoid OOM.
                // The original Supabase image is already high quality — use it as-is.
                image.setProcessedUrl(image.getOriginalUrl());
                image.setStatus(ImageStatus.PROCESSED);
                log.info("Image {} is remote — skipping local processing, using original URL", imageId);
            } else {
                // Development (local disk): run full processing pipeline
                Path originalPath = Paths.get(uploadPath).toAbsolutePath()
                        .resolve(image.getOriginalUrl().replace("/uploads/", ""));
                ...
                try {
                    log.info("Calling HF Space for image {}", imageId);
        ...
        if (output.isTextual()) {
            ...
            if (text.startsWith("data:")) {
                ...
                if (parts.length < 2 || parts[1].isBlank())
                    throw new RuntimeException("Malformed base64 data URL from HF Space");
    ...
    private String uploadToSupabase(byte[] imageBytes, String fileName) {
