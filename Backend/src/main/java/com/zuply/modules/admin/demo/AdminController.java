...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<SellerAdminDto>> rejectSeller(@PathVariable Long id) {
        return ResponseEntity.ok(
                ApiResponse.success("Seller rejected", adminService.rejectSeller(id)));
    }
    ...
    /**
     * Returns all products pending admin review, from both the AI-listing table
     * and the manually-created products table.
     */
    @GetMapping("/products")
    public ResponseEntity<ApiResponse<List<AdminProductDto>>> getAllProducts() {
        return ResponseEntity.ok(
    ...
    /**
     * Approve a product.
     * @param source LISTING (default) for AI/upload products, MANUAL for manually created products.
     */
    @PatchMapping("/products/{id}/approve")
    public ResponseEntity<ApiResponse<AdminProductDto>> approveProduct(
            @PathVariable Long id,
            @RequestParam(defaultValue = "LISTING") String source) {
        return ResponseEntity.ok(
                ApiResponse.success("Product approved and published to marketplace",
                        adminService.approveProduct(id, source)));
    }
    ...
    public ResponseEntity<ApiResponse<AdminProductDto>> rejectProduct(
            ...
            @RequestParam(defaultValue = "LISTING") String source) {
        return ResponseEntity.ok(
                ApiResponse.success("Product rejected", adminService.rejectProduct(id, source)));
    }
    ...
    public ResponseEntity<ApiResponse<List<AdminOrderDto>>> getAllOrders() {
        return ResponseEntity.ok(
                ApiResponse.success("Orders fetched successfully", adminService.getAllOrders()));
    }
