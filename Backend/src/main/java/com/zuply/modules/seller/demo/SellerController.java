...
public class SellerController {
    ...
    public ResponseEntity<ApiResponse<SellerDashboardDto>> getDashboard(Authentication authentication) {
        ...
        if (isSuspended(seller)) {
            return ResponseEntity.status(403).body(ApiResponse.failure("Your account has been suspended."));
        }
    ...
    public ResponseEntity<ApiResponse<SellerOrderDto>> updateOrderStatus(
        ...
        try {
            SellerOrderDto updatedOrder = sellerService.updateOrderStatus(seller.getId(), id, status);
