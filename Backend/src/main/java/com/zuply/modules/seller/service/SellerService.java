...
public class SellerService {
    ...
    private com.zuply.modules.product.repository.ProductRepository marketplaceProductRepository;
    ...
    public SellerDashboardDto getDashboard(Long sellerId) {
        long totalProducts = marketplaceProductRepository.findBySellerId(sellerId).size();
        ...
        long pendingOrders = sellerOrderItems.stream()
                .filter(item -> item.getOrder() != null && item.getOrder().getStatus() == OrderStatus.PLACED)
                .map(item -> item.getOrder().getId())
                .distinct()
                .count();
    ...
    public List<SellerOrderDto> getSellerOrders(Long sellerId) {
        ...
        for (OrderItem item : sellerOrderItems) {
            ...
            SellerOrderDto dto = new SellerOrderDto(
                    ...
                    productName,
                    item.getQuantity(),
                    order.getStatus() != null ? order.getStatus().name() : "PLACED"
            );
            orderDtos.add(dto);
        }
    ...
    public SellerOrderDto updateOrderStatus(Long sellerId, Long orderId, String newStatus) {
        ...
        // Find one of this seller's items for the product name
        String productName = sellerItems.stream()
                .filter(item -> item.getOrder().getId().equals(orderId))
                .findFirst()
                .map(item -> item.getProduct() != null ? item.getProduct().getName() : "Unknown")
                .orElse("Unknown");

        String customerName = saved.getCustomerName() != null
                ? saved.getCustomerName()
                : (saved.getCustomer() != null ? saved.getCustomer().getName() : "Unknown");

        int qty = sellerItems.stream()
                .filter(item -> item.getOrder().getId().equals(orderId))
                .findFirst()
                .map(OrderItem::getQuantity)
                .orElse(0);

        return new SellerOrderDto(saved.getId(), customerName, productName, qty, saved.getStatus().name());
    }
}
